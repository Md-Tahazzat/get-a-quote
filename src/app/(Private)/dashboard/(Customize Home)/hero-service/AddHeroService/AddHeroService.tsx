...
const AddHeroService: React.FC<AddModalProps> = ({ closeModal, isModalOpen,  getHeroService }) => {
  ...
  const handleSubmit = async (e: any) => {
    ...
    if (image) {
      ...
      if (imageUrl) {
        try {
          const response = await fetch(`${process.env.NEXT_PUBLIC_URL}/api/dashboard/home/homeService`, {
            method: 'POST',
            headers: {
  ...
  return (
    <div className={`fixed top-0 left-0 flex items-center justify-center w-full h-full bg-gray-800 bg-opacity-50 ${isModalOpen ? "block" : "hidden"
      ...
      <div className="bg-white p-6 rounded-lg w-full md:w-[50%]">
        <h2 className="text-2xl font-bold mb-4">Add services</h2>
