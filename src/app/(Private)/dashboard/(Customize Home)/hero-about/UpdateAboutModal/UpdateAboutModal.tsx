...
const UpdateAboutModal: React.FC<aboutUpdateModalProps> = ({ about, getAbout, updateModal, onClose }) => {
  const [updatedTitle, setUpdatedTitle] = useState(about.title);
  const [updatedSubtitle, setUpdatedSubtitle] = useState(about.subtitle);
  const [updatedImage, setUpdatedImage] = useState(about.image);
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const res = await fetch(
        `${process.env.NEXT_PUBLIC_URL}/api/dashboard/home/about/${about?.id}`,
        {
          method: 'PUT',
          headers: {
